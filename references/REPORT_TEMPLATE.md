...
### 같은 회사의 여러 응답
Private Organization Group ID가 같은 진단이 여러 개면 다음도 보여줍니다.
- 조직/Scope 평균·중앙값
- 점수 분포와 범위
...
### CAMP Bench
진단 결과가 제출되었고 표본이 충분하면 다음을 보여줍니다.
- Overall Percentile
- Industry Percentile
- Peer Group Percentile
...
**Peer Group** = Industry Segment, 규모, 국가/지역, Scope, Function이 유사한 익명 조직군입니다. Cohort가 10개 조직 미만이면 더 넓은 그룹으로 비교하거나 `표본 부족`으로 표시합니다.

참여자는 자기 회사명을 볼 수 있지만 다른 참여기업의 회사명과 개별 점수는 익명화합니다. Named Company 비교는 공개 Evidence 또는 명시적으로 허용된 데이터만 사용합니다.
